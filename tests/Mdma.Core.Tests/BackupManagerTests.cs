...
public class BackupManagerTests
{
    ...
    public void CreateBackup_Ndm_Captures_NeatDb_And_Task_Directory()
    {
        var fixture = new NdmFixtureBuilder(_testDir)
            .WithTask(521, "f.bin", "https://example.com/f.bin", 1000, (0, 999, 500));
        fixture.Build();
        ...
        Assert.That(File.Exists(Path.Combine(handle.StoragePath, "task", "521", "seg.x0")), Is.True);
        Assert.That(File.Exists(Path.Combine(handle.StoragePath, "task", "521", "segments.bin")), Is.True);
    ...
    public void CreateBackup_Ndm_Copies_Are_Byte_For_Byte_Identical()
    {
        ...
        var backedUpDbBytes = File.ReadAllBytes(Path.Combine(result.Value!.StoragePath, "neatdb.db"));
    ...
    public void CreateBackup_Ndm_Succeeds_When_Task_Directory_Does_Not_Exist()
    {
        ...
        var result = manager.CreateBackup(location, _workingRoot, taskNativeId: "999-does-not-exist");
    ...
    public void CreateBackup_Jd2_Captures_Newest_Zip()
    {
        var fixture = new Jd2FixtureBuilder(_testDir, counter: 5)
            .WithLink("99", "00", "f.bin", "https://example.com/f.bin", 1000, 200, 200);
        fixture.Build();
        ...
        Assert.That(result.IsSuccess, Is.True);
        Assert.That(File.Exists(Path.Combine(result.Value!.StoragePath, "downloadList5.zip")), Is.True);
    }
    ...
    public void CreateBackup_Jd2_Picks_Newest_When_Multiple_Zips_Present()
    {
        ...
        Assert.That(result.IsSuccess, Is.True);
        Assert.That(File.Exists(Path.Combine(result.Value!.StoragePath, "downloadList5.zip")), Is.True);
        Assert.That(File.Exists(Path.Combine(result.Value!.StoragePath, "downloadList1.zip")), Is.False);
    }
    ...
    public void ListBackups_Returns_Newest_First()
    {
        ...
        var clock = new FakeClock { UtcNow = new DateTimeOffset(2026, 1, 1, 0, 0, 0, TimeSpan.Zero) };
    ...
    {
        var ndmFixture = new NdmFixtureBuilder(Path.Combine(_testDir, "ndm"))
            .WithTask(1, "f.bin", "https://example.com/f.bin", 1000, (0, 999, 500));
        ndmFixture.Build();
        var ndmLocation = new TargetAppLocation(TargetApp.NDM, ndmFixture.TempDirectory, Path.Combine(_testDir, "ndm"), DownloadDirectory: null, WasAutoDetected: true);

        var jd2Fixture = new Jd2FixtureBuilder(Path.Combine(_testDir, "jd2"))
            .WithLink("99", "00", "f.bin", "https://example.com/f.bin", 1000, 200, 200);
        jd2Fixture.Build();
        var jd2Location = new TargetAppLocation(TargetApp.JD2, jd2Fixture.CfgDirectory, MetadataDir: null, DownloadDirectory: null, WasAutoDetected: true);
