...
public class RevertManagerTests
{
    ...
    public void Revert_Fails_When_Snapshot_File_Tampered_And_Restores_Nothing()
    {
        ...
        File.WriteAllText(Path.Combine(backup.StoragePath, "neatdb.db"), "tampered snapshot content");
        ...
        Assert.That(File.ReadAllText(fixture.NeatDbPath), Is.EqualTo("live file, should remain exactly this"));
    }
    ...
    public void Revert_Fails_Cleanly_When_Manifest_Missing()
    {
        ...
        var handle = new BackupHandle("fake-id", TargetApp.NDM, DateTimeOffset.UtcNow, fakeStoragePath);
    ...
    {
        var fixture = new NdmFixtureBuilder(_testDir)
            .WithTask(1, "f.bin", "https://example.com/f.bin", 2000, (0, 999, 500), (1000, 1999, 500));
        fixture.Build();
        var seg0Path = Path.Combine(fixture.TempDirectory, "1", "seg.x0");
