...
public sealed class Jd2Exporter : IMdmaExporter
{
    ...
    )
    {
        ...
        catch (Exception ex)
        {
            return new MdmaError(
                ...
                Details: sourceLocation.InstallOrConfigDir,
                Inner: ex
            );
        }
        ...
        try
        {
            ...
            downloadFolder = packageEntry is not null
                ? ReadJsonString(packageEntry, "downloadFolder")
                : null;
            ...
            if (linkEntry is null)
            {
                return new MdmaError(
                    MdmaErrorCode.ExportFailed,
                    "Link entry not found in downloadList*.zip.",
                    Details: task.NativeId
                );
            }
            ...
            chunkProgress =
                root.TryGetProperty("chunkProgress", out var cp)
        ...
        catch (Exception ex)
        {
            return new MdmaError(
                MdmaErrorCode.ExportFailed,
                "Failed to read task metadata from downloadList*.zip.",
                Details: newestZip,
                Inner: ex
            );
        }
        ...
        var sourceFilePath =
            File.Exists(partPath) ? partPath
            : File.Exists(completePath) ? completePath
            : null;
        if (sourceFilePath is null && task.DownloadedBytes > 0)
        {
            return new MdmaError(
                MdmaErrorCode.ExportFailed,
        ...
        catch (Exception ex)
        {
            return new MdmaError(
                MdmaErrorCode.ExportFailed,
                "Could not create staging directory for chunk slicing.",
                Details: stagingDir,
                Inner: ex
            );
        }
        ...
        catch (Exception ex)
        {
            ...
            return new MdmaError(
                MdmaErrorCode.ExportFailed,
                "Failed to slice chunk data from the source file.",
    ...
    private static List<MdmaChunkSource> SliceChunks(
    ...
    )
    {
        ...
        using var sourceStream =
            !string.IsNullOrEmpty(sourceFilePath) && File.Exists(sourceFilePath)
        ...
        for (int i = 0; i < chunkCount; i++)
        {
            ...
            long downloadedInChunk = Math.Clamp(
                absoluteProgress - startByte,
                0,
                endByte - startByte + 1
            );
            ...
            {
                if (downloadedInChunk > 0 && sourceStream is not null)
                {
    ...
    private static void CopyExactly(Stream source, Stream destination, long byteCount)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (read == 0)
                break; // source shorter than expected (sparse hole or truncated file) -- stop, don't fabricate data
    ...
    private static void TryDeleteDirectoryBestEffort(string dir)
    {
        ...
        { /* best-effort cleanup of staged chunk slices */
        }
    }
}
